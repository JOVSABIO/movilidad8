...
def create_advanced_map(complete_data):
    ...
    for _, row in complete_data.iterrows():
        gravedad = str(row['gravedad']).lower()
        color = 'gray'
        
        for key, value in color_map.items():
            if key.lower() in gravedad:
                color = value
                break
    ...
    if not st.session_state.filters_applied:
        ...
        if not filtered_map.empty:
            ...
            if st.session_state.map_type == "Marcadores":
                ...
                folium_map = create_advanced_map(filtered_complete)
            else:
                folium_map = create_folium_map(filtered_map, 'markers', st.session_state.zoom_level, center=medellin_center)
            st_folium(folium_map, width=1500, height=700, returned_objects=[])
            st.markdown('</div>', unsafe_allow_html=True)
        else:
